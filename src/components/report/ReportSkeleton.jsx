function SkeletonLine({ className = "" }) {
  return <div className={`h-4 rounded-md bg-[var(--color-muted)] animate-pulse ${className}`} />;
}

function SkeletonCard({ children, className = "" }) {
  return (
    <div className={`rounded-lg border border-[--color-border] bg-[var(--color-surface)] p-4 ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <SkeletonCard className="space-y-3">
        ...
        <div className="space-y-3">
          {[0, 1, 2].map((item) => (
            <div key={item} className="rounded-lg bg-[var(--color-surface-alt)] p-4">
              <SkeletonLine className="w-11/12" />
